...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    def stripe_amount(self, request, pk):
        try:
            ...
            return Response(
                "No payment found. Your registration may have timed out. Cancel your current registration and start again.",
                status=404,
            )
    ...
    def payment_intent(self, request, pk):
        try:
            ...
            stripe_payment = calculate_payment_amount(amount_due)
            stripe_amount_due = int(
                round_half_up(stripe_payment[0] * 100)
            )  # total (with fees) in cents

            if amount_due > 0 and (
                player.stripe_customer_id is None or player.stripe_customer_id.strip() == ""
            ):
                customer = stripe.Customer.create()
                player.stripe_customer_id = customer.id
                player.save()
            ...
            intent = stripe.PaymentIntent.create(
                ...
                description="Online payment for {} ({}) by {}".format(
                    event.name, event.start_date.strftime("%Y-%m-%d"), user.get_full_name()
                ),
                metadata={
            ...
            logger.info(
                "Payment intent created", payment_id=pk, intent_id=intent.id, status=intent.status
            )
    ...
    def issue_refunds(self, request):
        ...
        for refund in refunds:
            try:
                ...
                result = Refund.objects.create_refund(
                    request.user, payment, refund_amount, refund["notes"]
                )
                update_to_unpaid(refund["refund_fees"])
                successful_refunds.append(
                    f"Refund of {result.refund_amount} created for {result.payment.id}"
                )
            except Exception as e:
                message = f"Refund failed for {refund}: {str(e)}"
