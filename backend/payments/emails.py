...
def send_payment_notification(payment, registration, player):
    ...
    event = payment.event
    logger.info(
        "Running send_payment_notification",
        email=user.email,
...
def send_member_welcome(user):
    ...
    send_templated_mail(
        ...
        context={
            ...
            "year": current_season(),
            "account_url": f"{settings.WEBSITE_URL}/my-account",
            "matchplay_url": f"{settings.WEBSITE_URL}/match-play",
...
def send_new_member_notification(user, player, notes):
    send_templated_mail(
        ...
        context={
            ...
            "ghin": player.ghin,
            "notes": notes,
            "admin_url": f"{settings.ADMIN_URL}/admin/register/player/?q={user.email}",
            "logo_image": inline_image,
        },
        template_suffix="html",
        headers={"Reply-To": "no-reply@bhmc.org"},
...
def send_event_confirmation(user, event, registration, payment):
    ...
    email_context = {
        ...
        "required_fees": f"${required_fees:,.2f}",
        "optional_fees": f"${optional_fees:,.2f}",
        "transaction_fees": f"${payment.transaction_fee:,.2f}",
        "total_fees": f"${required_fees + optional_fees + payment.transaction_fee:,.2f}",
...
def send_refund_notification(payment, refund):
    ...
    email_context = {
        ...
        "event_name": event.name,
        "event_date": event.start_date,
        "total_refund": f"${refund.refund_amount:,.2f}",
