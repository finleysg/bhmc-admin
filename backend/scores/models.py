...
class EventScoreCard(models.Model):
    ...
    handicap_index = models.DecimalField(
        verbose_name="Handicap Index", max_digits=4, decimal_places=2, blank=True, null=True
    )
    ...
    class Meta:
        ...
        constraints = [UniqueConstraint(fields=["event", "player"], name="unique_event_scorecard")]

    objects = EventScoreCardManager()

    def __str__(self):
        return f"{self.event} {self.player} {self.course}"


class EventScore(models.Model):
    scorecard = models.ForeignKey(
        verbose_name="Scorecard", to=EventScoreCard, related_name="scores", on_delete=CASCADE
    )
