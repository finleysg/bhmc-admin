...
export default function CashPayoutPage() {
	...
	const fetchPayouts = useCallback(async () => {
		try {
			const response = await fetch(`/api/events/${eventId}/payouts?payoutType=Cash`)
			if (!response.ok) {
			...
			console.error("Error fetching cash payouts:", err)
			setError("Failed to load cash payouts")
		} finally {
			setLoading(false)
		}
	}, [eventId])
	...
	const handleMarkPaid = async () => {
		...
		try {
			const response = await fetch(`/api/events/${eventId}/payouts/mark-paid`, {
				...
				body: JSON.stringify({ payoutType: "Cash" }),
			})
			if (!response.ok) {
			...
			setMarked(true)
			setFeedback({ type: "success", message: "Cash payouts marked as paid" })
		} catch (err) {
	...
	if (payouts.length === 0) {
		return (
			<PageLayout maxWidth="3xl">
				<EmptyState message="No cash payouts found for this event." />
			</PageLayout>
		)
	}
