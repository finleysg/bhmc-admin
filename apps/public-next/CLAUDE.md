...
- `lib/django-auth.ts` — client-side auth functions (login, logout, getCurrentUser) that call Django directly.
- `app/api/auth/[...path]/route.ts` — Next.js API route that proxies auth requests to Django and forwards `Set-Cookie` headers.
...
- `NEXT_PUBLIC_DJANGO_URL` — Django URL for client-side calls (default: `http://localhost:8000`)
