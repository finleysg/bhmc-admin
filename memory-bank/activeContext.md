...
- **Zod Schema Generation**: Added insert and update Zod schemas for every table using createInsertSchema and createUpdateSchema from drizzle-zod; exported schemas for each table in models files matching their schema files (courses: course, hole, tee; events: event, eventFee, etc.; golf-genius: integrationLog; registration: payment, refund, etc.; scores: eventScore, eventScorecard); enables type-safe validation in repositories and services.
