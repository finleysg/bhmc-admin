...
- **Admin Registration Enhancements**: Complete admin registration workflow improvements including POST `/:eventId/reserve-admin-slots` endpoint for administrative slot reservation with transaction safety and validation, and transaction fee calculation using Stripe-like formula (2.9% + $0.30) in `calculateTransactionFee` domain function with comprehensive test coverage; automatically calculates fees when `collectPayment` is true during registration completion
